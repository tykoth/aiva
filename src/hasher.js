...
// use hash string and hashMap of callback functions
// for correct identification and execution.
// Is set as global.
...
const hashMap = {}
  // !for future multiple replies: don't delete on 'handle',
  // but do a periodic scan and prune keys with older hash timestamp

/**
 * Generates a hash string as current _.uniqueId(_.now()), format: <scriptname>_<timestamp>_<uniqId>
 * Used when client passes msg to target script.
 * This generates and registers in the hashMap the hash string,
 * and also the optional callback function to handle replies.
...
function gen(id = 'unidentified', cb) {
...
/**
 * Handler for socket client; use the msg's hash to find
 * a callback function from hashMap, and execute cb(msg) if found.
 * Note the payload msg received must contain the same hash,
 * thus it's your job to include the same when replying.
